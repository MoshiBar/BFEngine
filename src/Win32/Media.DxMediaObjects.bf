...
{
	public static class DxMediaObjects
	{
		// --- Constants ---
		...
		public struct IEnumDMO : IUnknown
		{
			...
			public HResult Next(uint32 cItemsToFetch, Guid* pCLSID, char16** Names, out uint32 pcItemsFetched) mut => VT.Next(ref this, cItemsToFetch, pCLSID, Names, out pcItemsFetched);
			...
			public struct VTable : IUnknown.VTable
			{
				public new function [CallingConvention(.Stdcall)] HResult(ref IEnumDMO self, uint32 cItemsToFetch, Guid* pCLSID, char16** Names, out uint32 pcItemsFetched) Next;
		...
		public static extern HResult DMORegister(char16* szName, in Guid clsidDMO, in Guid guidCategory, uint32 dwFlags, uint32 cInTypes, in DMO_PARTIAL_MEDIATYPE pInTypes, uint32 cOutTypes, in DMO_PARTIAL_MEDIATYPE pOutTypes);
		[Import("msdmo.dll"), CLink, CallingConvention(.Stdcall)]
